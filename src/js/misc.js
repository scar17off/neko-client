...
export const KeyCode = {
	...
	BACKTICK: 192, TILDE: 192, MINUS: 173, UNDERSCORE: 189,
	EQUALS: 187, PLUS: 61, L_BRACKET: 219, L_CURLY: 219,
...
// 	let tip = null;
// 	function tooltip() {
// 		let epos = element.getBoundingClientRect();
// 		let y = epos.top + epos.height / 2;
// 		tip = mkHTML('span', {
// 			innerHTML: message,
// 			className: 'framed tooltip whitetext'
// 		});
// 		document.body.appendChild(tip);
// 		let tpos = tip.getBoundingClientRect();
// 		y -= tpos.height / 2;
// 		let x = epos.left - tpos.width - elementSpacing;
// 		if (x < elementSpacing) {
...
// 		}
// 	};
// 	/*let observer = new MutationObserver(e => { // Why does this not fire at all?
...
/*function decompressu16(input) {
	let originalLength = (((input[1] & 0xFF) << 8 | (input[0] & 0xFF)) + 1) * 2;
	let output = new Uint8Array(originalLength);
	let numOfRepeats = (input[3] & 0xFF) << 8 | (input[2] & 0xFF);
	let offset = numOfRepeats * 2 + 4;
	let uptr = 0;
	let cptr = offset;
	for (let i = 0; i < numOfRepeats; i++) {
		let currentRepeatLoc = 2 * ((((input[4 + i * 2 + 1] & 0xFF) << 8) | (input[4 + i * 2] & 0xFF)))
				+ offset;
		while (cptr < currentRepeatLoc) {
			output[uptr++] = input[cptr++];
		}
		let repeatedNum = ((input[cptr + 1] & 0xFF) << 8 | (input[cptr] & 0xFF)) + 1;
		let repeatedColorRGB = (input[cptr + 3] & 0xFF) << 8 | (input[cptr + 2] & 0xFF);
		cptr += 4;
		while (repeatedNum-- != 0) {
