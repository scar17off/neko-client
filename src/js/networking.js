...
import { decompress } from './misc.js';
...
import { mouse } from './canvas_renderer.js';
...
export class OldProtocolImpl extends Protocol {
	...
	updatePixel(x, y, rgb, undocb) {
		let distx = Math.floor(x / OldProtocol.chunkSize) - Math.floor(this.lastSentX / (OldProtocol.chunkSize * 16)); distx *= distx;
		let disty = Math.floor(y / OldProtocol.chunkSize) - Math.floor(this.lastSentY / (OldProtocol.chunkSize * 16)); disty *= disty;
