!function () {
	...
	var windowClass = new OWOP.client.windowSys.class.window("Coordinates Saver", options, windowFunc);
	OWOP.client.windowSys.addWindow(windowClass).move(window.innerHeight / 3, window.innerWidth / 3);
	...
	class Coordinate {
		constructor(id) {
			...
			// document.getElementById('coord-' + this.id).onclick = () => OWOP.client.camera.centerCameraTo(parseInt(document.getElementById('x-' + this.id).textContent), parseInt(document.getElementById('y-' + this.id).textContent));
			document.getElementById('coord-' + this.id).onclick = () => {
				var x = parseInt(document.getElementById('x-' + this.id).innerText);
				var y = parseInt(document.getElementById('y-' + this.id).innerText);
				OWOP.chat.local(`/tp ${x} ${y}`);
			}
		}
	}
