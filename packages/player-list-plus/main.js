!function () {
	let plt = OWOP.utils.playerListTable;
	let pl = OWOP.utils.playerList;
	let pls = OWOP.misc.world.players;
	let sn = false;
	let cm = OWOP.createContextMenu;
	// let tp = OPM.require('core-utils').chat.commands.tp;
	let tp = function (text) {
		OWOP.chat.local("/tp " + text);
	}
	let ui, fi, tr, id, rs = false;
	let is;
	...
	style.innerHTML = `
		#player-list>table>tr>td:nth-child(3){
			border-right:1px solid rgba(0,0,0,0.3);
		}
		#player-list>table>tr>td:nth-child(4){
			border-right:1px solid rgba(0,0,0,0.3);
		}
		#player-list>table>tr>td:nth-child(5){
			border-right:1px solid rgba(0,0,0,0.3);
		}
	`
	...
	OWOP.client.eventSys.on(OWOP.client.EVENTS.net.disconnected, function () {
		disconnected = true;
	});
	OWOP.client.eventSys.on(OWOP.client.EVENTS.misc.worldInitialized, function () {
		pls = OWOP.misc.world.players;
		disconnected = false;
	});
		...
		Object.keys(pl).forEach(p => {
			if (sn) {
				...
				if (pl[p].children.length !== 6) {
					...
					pl[p].innerHTML += `<td>${pls[p].tool.name}</td><td><div style="width:15px;height:15px;background-color:rgb(${pls[p].rgb.join(',')})" title="${pls[p].rgb.join(', ')}" onclick="color(${pls[p].rgb})"></div></td>`;
